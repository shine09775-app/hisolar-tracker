-- Hi Solar Tracker
-- LINE auth RLS app scoping migration
-- Forward-only and idempotent: binds authenticated RLS helpers to the
-- JWT organization claim while preserving database-backed membership checks.
  ...
  select
    ...
    or exists (
      ...
      where requester.user_id = auth.uid()
        and requester.organization = public.current_request_organization()
...
as $$
  select
    target_user_id = auth.uid()
    or (
      target_org = public.current_request_organization()
      and public.can_manage_organization(target_org)
    );
$$;
...
to authenticated
with check (
  organization = public.current_request_organization()
  and public.can_manage_organization(organization)
);
...
with check (
  user_id = auth.uid()
  and requested_organization = public.current_request_organization()
  and status = 'pending'
);
...
to authenticated
using (
  requested_organization = public.current_request_organization()
  and public.can_manage_organization(requested_organization)
)
with check (
  requested_organization = public.current_request_organization()
  and public.can_manage_organization(requested_organization)
...
with check (
  job_id is not null
  and public.current_request_organization() in ('hisolar', 'jdk')
