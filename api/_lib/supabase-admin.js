...
function getSupabaseAdminClient() {
  ...
  // Both values become HTTP header values (apikey / Authorization) on every
  // admin request, so they are read through the env guard.
  const url = readCleanEnv('SUPABASE_URL', { pattern: /^https:\/\/[^\s]+$/ });
  const serviceKey = readCleanEnv('SUPABASE_SERVICE_ROLE_KEY');
